    ...
    <main className="min-h-dvh overflow-hidden bg-canvas">
      <nav className="relative z-30 border-b border-white/10 bg-[#020512]/90 backdrop-blur-xl" aria-label="Public navigation">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4 lg:px-10">
          ...
          <div className="flex items-center gap-2"><Link href="/sign-in" className="hidden rounded-lg border border-white/15 px-4 py-2 text-xs font-bold text-text sm:block">Sign In</Link><Link href="/sign-up" className="rounded-lg bg-accent px-4 py-2 text-xs font-black text-white shadow-[0_0_30px_rgba(229,18,43,.3)]">GET RUNNER ACCESS →</Link></div>
        </div>
      </nav>
      <RunnerTicker />
      ...
      <section className="hero-stadium relative border-b border-white/10">
        <div className="mx-auto grid min-h-[760px] max-w-[1500px] items-center gap-12 px-5 py-20 lg:grid-cols-[1.05fr_.95fr] lg:px-10">
          <div className="relative z-10">
            ...
            <h1 className="mt-7 max-w-4xl text-[clamp(3.8rem,8vw,8.6rem)] font-black uppercase leading-[.78] tracking-[-.075em] text-white">WE RUN<br /><span className="runner-outline">SPORTS</span><br /><span className="text-accent">&amp; ANALYTICS.</span></h1>
      ...
      <section className="relative overflow-hidden border-y border-accent/25 bg-accent"><div className="cta-stripes absolute inset-0 opacity-20" /><div className="relative mx-auto flex max-w-[1500px] flex-col gap-8 px-5 py-16 lg:flex-row lg:items-center lg:justify-between lg:px-10"><div><p className="text-[10px] font-black uppercase tracking-[.25em] text-white/70">werunsportsandanalytics.com</p><h2 className="mt-2 text-4xl font-black uppercase tracking-tight text-white sm:text-6xl">THE NAME IS THE MISSION.</h2></div><div className="flex flex-wrap gap-3"><Link href="/pricing" className="shrink-0 rounded-xl border border-white/40 px-6 py-4 text-sm font-black text-white">VIEW ACCESS</Link><Link href="/sign-up" className="shrink-0 rounded-xl bg-white px-6 py-4 text-sm font-black text-accent shadow-2xl">BUILD YOUR EDGE →</Link></div></div></section>
      <footer className="mx-auto max-w-[1500px] px-5 py-10 lg:px-10"><div className="flex flex-col gap-5 border-b border-white/10 pb-8 sm:flex-row sm:items-center sm:justify-between"><RunnerLogo /><p className="text-xs font-black uppercase tracking-[.2em] text-text-subtle">WE RUN SPORTS &amp; ANALYTICS.</p></div><div className="pt-7"><ResponsibleGamblingNotice /><p className="mt-4 text-[10px] text-text-subtle">© 2026 Runner Sports &amp; Analytics LLC · Research and analytics only · Not a sportsbook</p></div></footer>
    </main>
  );
}
