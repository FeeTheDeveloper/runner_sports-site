...
export default async function TeamsPage() { const teams = (await Promise.all(LEAGUES.map(league => getTeamRegistry(league).catch(()=>[])))).flat(); const grouped = teams.reduce<Record<string, typeof teams>>((result, team) => { const letter = team.name.charAt(0).toUpperCase(); (result[letter] ??= []).push(team); return result; }, {}); return <div className="space-y-7"><ProductHeading eyebrow="Team Lab" title="Team Intelligence" description="Canonical team identities connect ESPN facts, sportsbook names, schedules, rosters, injuries, standings, and Runner model history."/>
  <div className="flex flex-wrap gap-1">{"ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map(letter=>grouped[letter] ? <a href={`#${letter}`} key={letter} className="grid h-8 w-8 place-items-center rounded border border-border bg-surface text-[10px] font-bold text-text-muted hover:border-accent hover:text-text">{letter}</a> : <span key={letter} title="No teams" aria-disabled="true" className="grid h-8 w-8 cursor-not-allowed place-items-center rounded border border-dashed border-border bg-surface text-[10px] font-bold text-text-subtle opacity-40">{letter}</span>)}</div>
