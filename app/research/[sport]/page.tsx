...
export default async function SportResearchPage({ params }: { params: Promise<{ sport: string }> }) {
  ...
  const counts = records.reduce<Record<string, number>>((map, record) => { map[record.dataType] = (map[record.dataType] ?? 0) + 1; return map; }, {});
  return <div className="space-y-7"><ProductHeading eyebrow="Research Center" title={`${label} Intelligence`} description={`ESPN facts and Runner market intelligence organized for ${label} research.`}/><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{["scoreboard","standings","injuries","roster"].map(type=><div key={type} className="runner-card p-5"><p className="text-[10px] font-bold uppercase tracking-widest text-text-subtle">{type}</p><p className="metric-number mt-3 text-3xl font-black text-text">{counts[type] ?? 0}</p><p className="mt-1 text-xs text-text-muted">records available</p></div>)}</div><div className="grid gap-3 md:grid-cols-3"><ResearchLink href="/games" title="Matchups"/><ResearchLink href="/teams" title="Teams"/><ResearchLink href="/markets" title="Market Trends"/></div></div>;
}
